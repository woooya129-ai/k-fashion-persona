...
if [ -f "./.env" ]; then
    ...
    echo "조치: ~/secrets/k-fashion/.env로 이동"
    exit 1
fi

# 2. ~/secrets에 .env가 있어야 함
if [ ! -f "$HOME/secrets/k-fashion/.env" ]; then
    echo "WARN: ~/secrets/k-fashion/.env가 없습니다"
...
if [ -f "$HOME/secrets/k-fashion/.env" ]; then
    if command -v stat &> /dev/null; then
        perms=$(stat -f "%A" "$HOME/secrets/k-fashion/.env" 2>/dev/null || stat -c "%a" "$HOME/secrets/k-fashion/.env" 2>/dev/null || echo "unknown")
        if [ "$perms" != "600" ] && [ "$perms" != "unknown" ]; then
            echo "WARN: ~/secrets/k-fashion/.env 권한이 600이 아닙니다 (현재: $perms)"
            echo "조치: chmod 600 ~/secrets/k-fashion/.env"
        fi
    fi
fi
