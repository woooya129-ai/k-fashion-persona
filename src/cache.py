...
def compute_concept_hash(concept_text: str, category: str, price_krw: int) -> str:
    """lock-in §5.3.1 concept_hash.

    M2 해소 (current-update-review 2026-05-01): 이름은 'concept_hash' 지만
    payload 는 (concept_text + category + price_krw) 3개 입력의 합쳐진 hash 다.
...
def compute_price_context_hash(
    ...
    payload = {
        "source": source,
        "period": period,
        "denominator_krw": int(denominator_krw),
